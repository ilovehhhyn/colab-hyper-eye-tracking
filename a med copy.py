...
COMPETITIVE MODE ONLY: Medium difficulty, collaborative scoring
"""
...
grid_positions = []
trial_results = []
game_sync_data = {'round': 0, 'target_pos': 0, 'condition': [], 'responses': {}, 'a_ready': False, 'b_ready': False}
...
print("=" * 60)
print("COMPUTER A - COMPETITIVE MEMORY GAME")
print("=" * 60)
print(f"Local IP: {LOCAL_IP}")
print(f"Remote IP: {REMOTE_IP}")
print("COMPETITIVE MODE: Medium difficulty, collaborative scoring")

# Set up EDF data file name
edf_fname = 'COMP_A_COMPETITIVE'
...
def load_all_images():
    """Load all images from stimuli folder - PNG support and correct plurals"""
    global images
    ...
    for category_key, folder_name in category_folders.items():
        ...
        if os.path.exists(folder_path):
            for i in range(10):  # Load 10 images per category (0-9)
                image_name = f"{folder_name}-{i}.png"  # PNG extension and plural form
...
# Gaze markers - Different colors for Computer A
...
def generate_round_parameters():
    """Generate round parameters and send to Computer B"""
...
def run_competitive_round():
    """Run a single competitive round - two-player collaborative mode"""
...
while not game_sync_data.get('b_ready', False):
    if time.time() - start_wait > 30:  # 30 second timeout
        print("Timeout waiting for Computer B")
        show_msg(win, "Timeout waiting for Computer B.\nPress any key to exit.")
        terminate_task()
    time.sleep(0.1)

print("✓ Both computers ready, starting game")
...
try:
    ...
    if recording_success:
        ...
        print("✓ Recording active - starting collaborative memory game")
        
        # Show game start message
        start_msg = f"Collaborative Memory Game Starting!\n\n{total_rounds} rounds ahead (medium difficulty only).\n\nComputer A leads! You generate rounds!\n\nPress any key to start Round 1"
        ...
        completion_msg += f'COMPETITIVE VERSION:\n'
        completion_msg += f'• Medium difficulty only\n'
        completion_msg += f'• PNG image support with correct folder names\n'
        completion_msg += f'• Collaborative team scoring\n'
        completion_msg += f'• Computer A leads game flow\n'
        completion_msg += f'\nPress any key to exit'
