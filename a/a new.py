...
class RobustSyncServer:
    ...
    def wait_for_response(self, expected_type, timeout=10):
        """Wait for specific response with longer timeout"""
        ...
        while time.perf_counter() - start_time < timeout:
            try:
                ...
                continue
        print(f"A: Timeout waiting for {expected_type}")
        return None
...
def run_synchronized_experiment():
    ...
    for trial_num in range(total_trials):
        ...
        # ========== STAGE 1: DOT DISPLAY ==========
        print("A: Stage 1 - Dot Display")
        GAZE_SHARING_ACTIVE = True  # ACTIVATE gaze sharing
        
        # Send stage signal
        sync_server.send_message('stage_dot_display', {
        ...
        if not sync_ack:
            print("A: Warning - No sync ack for dot display")
        ...
        while not (response_received['server'] and response_received['client']):
            ...
            resp_msg = sync_server.wait_for_response('response_update', timeout=0.01)
