...
def load_conditions():
    """Load conditions from dyad_conditions.json - HARD MODE (8x8 logical)"""
    global conditions
    try:
        with open('dyad_conditions.json', 'r') as f:
            data = json.load(f)
            conditions = {
                'hard': data['top_layouts_array_fixed_64']  # Hard (64-element arrays for 8x8 patterns)
            }
        print("✓ Conditions loaded from dyad_conditions.json (hard difficulty - 8x8 logical)")
    except FileNotFoundError:
        print("dyad_conditions.json not found. Using default hard conditions.")
        # Default conditions if file not found - 64 elements for 8x8
        default_pattern = []
        for i in range(64):
            default_pattern.append(i % 4)  # Cycle through 0,1,2,3
        conditions = {
            'hard': [default_pattern]
        }
    except Exception as e:
        print(f"Error loading conditions: {e}")
        # Default conditions if error - 64 elements for 8x8
        default_pattern = []
        for i in range(64):
            default_pattern.append(i % 4)  # Cycle through 0,1,2,3
        conditions = {
...
def create_synchronized_grid(condition_array, seed):
    """Create 8x8 grid from condition array using shared seed - HARD MODE (8x8 logical)"""
    ...
    # Grid setup - 8x8 physical grid representing 8x8 logical pattern (1:1 mapping)
    ...
    # Direct 1:1 mapping - each logical position maps to one physical position
    for row in range(8):
        for col in range(8):
            logical_index = row * 8 + col  # Convert 2D to 1D index
            category = condition_categories[logical_index]
    ...
    print(f"✓ Synchronized grid created with seed {seed} (hard difficulty - 8x8 logical pattern)")
