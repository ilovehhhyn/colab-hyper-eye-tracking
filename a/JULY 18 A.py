...
# 3. MODIFY STAGE 2 in run_synchronized_experiment() (replace the entire Stage 2 section)
        ...
        while not (response_received['server'] and response_received['client']):
            ...
            resp_msg = sync_server.get_message(timeout=0.01)
            if resp_msg and resp_msg.get('type') == 'response_update':
                ...
                if resp_data.get('responder') == 'client':
                    if not response_received['client']:
                        ...
                        client_response = resp_data.get('response')  # FIX: Actually assign the response
                        client_rt = resp_data.get('rt')  # Also get the reaction time
