...
def create_game_grid():
    """Create the 8x8 game grid with 2x2 logical structure (each logical cell = 4x4 physical cells)"""
    global grid_stimuli, grid_covers, grid_positions, question_mark, score_text, timer_text
    
    # Grid setup - 8x8 physical grid representing 2x2 logical grid
    ...
    # Create image categories - 1 of each type for 4 logical positions
    categories = ['face', 'limb', 'house', 'car']
    all_items = categories.copy()  # Just one of each for 4 positions
    ...
    # Create 8x8 physical grid where each 4x4 block shows the same image
    ...
    print("✓ Game grid created (8x8 physical, 2x2 logical)")
...
task_msg += '• 8x8 physical grid = 2x2 logical grid (each logical cell = 4x4 identical images)\n'
task_msg += '• 4 unique logical positions (1 each: Face, Limb, House, Car)\n'
