    ...
    if sample is not None:
        ...
        if gaze_data and gaze_data[0] != pylink.MISSING_DATA and gaze_data[1] != pylink.MISSING_DATA:
            ...
            try:
                ...
                gaze_x = gaze_data[0] - scn_width/2 + 200
                gaze_y = scn_height/2 - 1.2 * gaze_data[1] + 50
...
def run_competitive_round():
    ...
    difficulty = game_sync_data['difficulty']
    
    # Create grid with the same condition as Computer A
    ...
    # Determine winner and scoring - CORRECTED LOGIC (same as Computer A)
...
while not calibration_successful:
    ...
    if validate_calibration():
        ...
        print("✓ Calibration and validation completed successfully")
    else:
        print("✗ Exiting due to calibration validation failure")
        terminate_task()

show_msg(win, "Calibration and validation complete!\n\nWaiting for Computer A to be ready...\n\nPress any key when both computers are ready.")
