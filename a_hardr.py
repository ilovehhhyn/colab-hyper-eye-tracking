...
def create_game_grid():
    """Create the 8x8 game grid with true 8x8 logical structure (each cell is unique)"""
    global grid_stimuli, grid_covers, grid_positions, question_mark, score_text, timer_text
    
    # Grid setup - 8x8 logical grid
    grid_size = 8
    cell_size = 70
    grid_spacing = 80
    ...
    # Create image categories - 16 of each type for 64 total positions
    ...
    # Create 8x8 grid where each cell can be different
    for row in range(grid_size):
        for col in range(grid_size):
            ...
            grid_stimuli.append({'rect': stimulus, 'text': text_stim, 'category': category, 'cell_idx': cell_idx})
    ...
    print("✓ Game grid created (8x8 logical)")
...
def run_competitive_round():
    ...
    target_cell_idx = grid_stimuli[target_position]['cell_idx']
    
    el_tracker.sendMessage(f"ROUND_{current_round}_START_TARGET_{target_position}_CELL_{target_cell_idx}_CATEGORY_{target_category}")
...
task_msg += '• 8x8 grid with 64 unique positions (16 each: Face, Limb, House, Car)\n'
