...
# Change these lines:
total_trials = 10  # Change from 5 to 10
grid_size = 8      # Add this new variable
cell_size = 80     # Update for larger grid  
grid_spacing = 90  # Update spacing
...
# Local gaze marker (Computer A's own gaze) - Blue theme (BIGGER SIZE)
local_gaze_marker = visual.Circle(win=win, radius=20, fillColor='deepskyblue', lineColor='navy', lineWidth=2)
local_gaze_sparkle1 = visual.Circle(win=win, radius=15, fillColor='lightblue', lineColor='white', lineWidth=1)

# Remote gaze marker (Computer B's gaze) - Red theme (BIGGER SIZE)
remote_gaze_marker = visual.Circle(win=win, radius=20, fillColor='tomato', lineColor='darkred', lineWidth=2)
remote_gaze_sparkle1 = visual.Circle(win=win, radius=15, fillColor='lightcoral', lineColor='white', lineWidth=1)

# Status display (smaller to make room for game)
status_background = visual.Rect(win=win, width=scn_width*0.9, height=60, 
                               fillColor='navy', lineColor='lightblue', lineWidth=2,
                               pos=[0, scn_height//2 - 40])
status_text = visual.TextStim(win, text='', pos=[0, scn_height//2 - 40], color='lightcyan', 
                             height=12, bold=True)
...
legend_text = visual.TextStim(win, text='Blue=A(You) Red=B(Remote)\nF=Face L=Limbs H=House C=Car', 
                             pos=[scn_width//2 - 150, -scn_height//2 + 120], 
...
# Add this line after "print(f"✓ Visual elements created")" and before calibration
load_images()
...
def update_local_gaze_display():
    ...
    if sample is not None:
        ...
        if gaze_data and gaze_data[0] != pylink.MISSING_DATA and gaze_data[1] != pylink.MISSING_DATA:
            ...
            # Send invalid data to Computer B
...
task_msg += '• Track your eye gaze (blue markers)\n'
task_msg += '• Send your gaze data to Computer B\n'
task_msg += '• Receive and display Computer B\'s gaze (red markers)\n'
...
try:
    ...
    if recording_success:
        ...
        while True:
            ...
            status_text.setText(
                f"COMPUTER A - FREE GAZE SHARING | Duration: {session_duration:.1f}s\n"
            ...
            # Draw gaze markers - local (blue) and remote (red)
            local_gaze_marker.draw()
            local_gaze_sparkle1.draw()
        ...
        completion_msg = f'Computer A - Session Complete!\n\n'
